...
### 11. 禁止异常信息exceptions传递到析构函数destructor外
...
遗憾的是**没有办法在析构函数内部区分出这两种情况**

因此在写析构函数时你**必须保守地假设有异常被激活**

因为如果在**一个异常被激活的同时，析构函数也会抛出异常**，并导致程序控 制权转移到析构函数外，C++将调用 terminate 函数，它会终止你程序的运行，而且是立即终止，甚至连局部对象都没有被释放
